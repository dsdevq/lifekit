"""Curator core — queue-drain loop and consolidation cycle.

Entry point: ``lifekit curator daemon``.

Drains ``queue.jsonl`` on every wake-up and calls Claude CLI to extract
memorable facts into domain files in the lifekit instance root.

Path resolution (in priority order):
  1. Individual env overrides (LIFEKIT_QUEUE_FILE, LIFEKIT_DEAD_LETTER_FILE, …)
  2. LIFEKIT_ROOT (read by life_root())
  3. ~/.life/ default
"""
...
LIFE_DIR: Path = Path(os.getenv("LIFEKIT_LIFE_DIR", str(life_root())))
QUEUE_FILE: Path = Path(os.getenv("LIFEKIT_QUEUE_FILE", str(LIFE_DIR / "queue.jsonl")))
DEAD_LETTER_FILE: Path = Path(
    os.getenv("LIFEKIT_DEAD_LETTER_FILE", str(LIFE_DIR / "queue.dead-letter.jsonl"))
)
CONSOLIDATION_MARKER: Path = Path(
    os.getenv("LIFEKIT_CONSOLIDATION_MARKER", str(LIFE_DIR / ".last_consolidation"))
)
IMPROVEMENTS_FILE: Path = Path(
    os.getenv("LIFEKIT_IMPROVEMENTS_FILE", str(LIFE_DIR / "improvements.md"))
)
CONSOLIDATION_INTERVAL: int = int(
    os.getenv("LIFEKIT_CONSOLIDATION_INTERVAL_SECONDS", str(24 * 60 * 60))
)
WAKE_INTERVAL: int = int(os.getenv("LIFEKIT_WAKE_INTERVAL_SECONDS", "900"))
CLAUDE_BIN: str = os.getenv("LIFEKIT_CLAUDE_BIN", "claude")
CLAUDE_TIMEOUT: int = int(os.getenv("LIFEKIT_CLAUDE_TIMEOUT_SECONDS", "180"))
...
def loop(wake_interval: int = WAKE_INTERVAL) -> None:
    logger.info(
